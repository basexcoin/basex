...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BasexMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BasexMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BasexMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("basex-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BasexMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
